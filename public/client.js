...
async function requestUserPlaylists(count = 0, owned = 0) {
  ...
  let page = await promiseThrottle.add(
    sp.getUserPlaylists.bind(this, (options = { offset: count, limit: limit }))
  );
...
async function requestPlaylistTracks(playlist, count = 0) {
  ...
  let options = { offset: count, limit: limit };
  ...
  updateDisplay(
    "tracklistLog",
    playlist.name + " " + playlistID + " Track count: " + playlistTracks.length
  );
...
function removeXTracks() {
  ...
  let trackIds = CURR_TRACKS.map(t => {
    return t.id;
  });
  let trackUris = CURR_TRACKS.map(t => {
    return t.uri;
  });
...
function displayAppend(tracks) {
  ...
  tracks.forEach(t => {
    ...
    console.log(t);
    let div = document.createElement("div");
    let text = `${t.name}, ${t.artists[0].name}, ${t.playlist.name}\n`;
